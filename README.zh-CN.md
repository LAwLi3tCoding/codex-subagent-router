# Codex 子 Agent 路由器

[English](README.md) | [简体中文](README.zh-CN.md)

这是一套无第三方依赖、用户级生效的 Codex 子 Agent 路由策略。普通的
`default` Agent 始终继承当前会话手动选择的模型和推理强度；只有任务确实
需要指定能力层级时，才会路由到 Luna、Terra 或 Sol 专用角色。

## 路由模型

路由使用两个正交维度：模型家族由工作模式和权限边界决定，effort 由剩余工作
复杂度决定。

- Luna：处理范围闭合、成本敏感且有机械校验基准的任务，包括按完整可执行方案
  实施代码修改。
- Terra：处理只读探索、资料研究和证据综合。
- Sol：处理仍需临场判断的实现，以及评审、验证、架构和高风险最终综合。
- `default`：仍继承当前会话模型和 effort，但必须由路由凭证证明存在明确的
  同能力层级需求。

Canonical 角色统一使用 `{family}-{effort}` 命名。本版本支持：

| 家族 | Canonical effort |
| --- | --- |
| Luna | `medium`、`high`、`xhigh`、`max` |
| Terra | `low`、`medium`、`high`、`xhigh`、`max`、`ultra` |
| Sol | `low`、`medium`、`high`、`xhigh`、`max`、`ultra` |

当前 Codex 运行时元数据把 `ultra` 描述为自动任务委派。它是 Sol/Terra 在异常
困难且可独立拆分任务中的编排模式，不是高于 `max` 的推理深度；Luna 没有
Ultra 路由。
Luna Medium 是最低的具名 Luna 路由；运行时 low 标签仅用于如实显示用户手动
选择后由父会话继承的模型强度。

Luna 和 Terra 可以收集、转换或整理证据，但不能独立负责架构、安全、发布、
迁移或其他高风险决策。此类最终结论必须由 Sol 专用角色综合判断。

这套映射遵循 OpenAI 的 GPT-5.6 官方说明：Luna 面向成本敏感、高吞吐工作；
Terra 平衡能力与成本；Sol 是旗舰能力层级；`max` 是文档公开支持的最高推理
强度。参见 [OpenAI 官方模型指南](https://developers.openai.com/api/docs/guides/latest-model)。

全局策略先拆分工作，再决定是否委派。多个独立、非琐碎且已经就绪的工作流默认
并行；存在前后依赖、重叠写入、授权限制、重复上下文或过高集成成本时保持直接
执行或串行。父 Agent 仍负责最终集成和验证，不设置共享的固定并发数量。

## 如何选择模型与 effort

路由分成两步：先根据工作类型和权限边界选择模型家族，再根据该家族内剩余
工作的复杂度选择 effort。提高 effort 不能弥补模型家族选错的问题。

### 第一步：选择模型家族

| 候选项 | 何时选择 | 何时不能选择 |
| --- | --- | --- |
| 父 Agent 直接执行 | 工作琐碎、不存在有用的子任务，或命中明确的并行例外 | 存在有明确价值的闭合子任务，或多个独立、非琐碎的工作流已经就绪 |
| `default` | 子任务明确需要父会话当前选中的同一模型和 effort，并且没有更合适的具名角色 | 只是暂时不知道该选哪个角色，或父会话能力不足 |
| Luna（`gpt-5.6-luna`） | 范围、方案、接口、文件边界、禁止选择、验收标准和机械校验基准全部完整 | 需要开放探索、临场判断、高风险决策，或缺少精确校验基准 |
| Terra（`gpt-5.6-terra`） | 只读代码探索、资料研究、关系梳理或证据综合 | 子 Agent 需要修改文件，或负责架构、安全、迁移、发布等高风险结论 |
| Sol（`gpt-5.6-sol`） | 需要临场判断、非机械写入、评审、验证、架构、冲突解决或高风险最终决策 | 更便宜的闭合 Luna 任务或只读 Terra 任务已经完整满足边界 |

混合任务需要在权限边界处拆分。非琐碎且可以隔离的只读证据阶段默认交给
Terra。闭合条件可以由父 Agent 在任务过程中补齐，不要求用户原始输入一次写全；
一旦方案和机械校验完整，非琐碎机械实现默认交给最低适用的 Luna。只有仍需本地
判断的实现才交给 Sol；阶段切换时重新计算路由。

当多个已就绪工作流能够独立推进，并且每项都超过一次琐碎查找或修改时，父 Agent
默认并行执行；前后依赖、重叠写入、授权限制、重复上下文或集成成本抵消收益时除外。
普通并行使用多个常规具名角色，不要求 `ultra`。只有一个子 Agent 自身还需要编排
异常复杂的多工作流任务时，才使用 Ultra。

### 第二步：选择推理 effort

选择所有条件都能被正面证明的最低 effort。未知条件会阻止降档，不能把
“尚不清楚”当成“任务很简单”的证据。

| Effort | 复杂度信号 | 选择边界 |
| --- | --- | --- |
| `low` | 单个窄步骤、输入完整、风险低，并有一个精确验收检查 | Terra 和 Sol 提供具名角色；Luna Low 只用于观测 `default` 继承的父会话配置。 |
| `medium` | 有成熟模式的常规闭合多步骤工作，步骤之间几乎没有相互影响 | 各步骤已知，并且能够机械排除相邻错误选择时使用。 |
| `high` | 一个闭合范围内存在多条显式约束或边界条件 | 需要更深入推理，但原因、模块或决策之间没有实质性交互时使用。 |
| `xhigh` | 至少两个相互作用的复杂度信号，例如多个模块、多种可能原因、大量边界条件或冲突证据 | 需要处理交互关系，但整体仍是一个闭合工作流时使用。 |
| `max` | 最困难的单个闭合任务 | Luna 用于有精确校验基准的闭合重逻辑；Terra 用于最困难的只读综合；Sol 用于高风险判断或完整跨组件设计。 |
| `ultra` | 至少两个真正独立、适合自动委派的工作流 | 仅限 Sol 或 Terra。它是编排模式，不是高于 `max` 的质量等级，也不是重试层级。 |

### 模型家族与 effort 的 Canonical 组合

Luna 只有在满足完整闭合方案契约时才能承担代码实现，结果必须能够独立、机械
验证。

| 角色 | 典型任务 | 何时停止或升级 |
| --- | --- | --- |
| `luna-medium` | 已指定文件中的字面修改、重复转换、生成映射，或不存在实质交互规则的简单实现 | 出现多条相互作用规则，或任何必要决策缺失 |
| `luna-high` | 一个闭合子系统内的多条显式规则和边界条件 | 规则开始跨模块相互作用，或预期结果不再精确 |
| `luna-xhigh` | 跨多个指定文件的显式交互规则，且所有决策和预期结果已经固定 | 实现重新打开设计、出现未解决原因，或风险升高 |
| `luna-max` | 最困难的闭合实现：包含大量相互作用的分支、状态或业务规则，并有精确校验基准 | 范围、方案或验证不再完整；切换到合适的 Sol 角色 |

Terra 永远只读。提高 Terra effort 只增加研究深度，不增加写入或最终决策权限。

| 角色 | 典型任务 | 何时停止或升级 |
| --- | --- | --- |
| `terra-low` | 精确查找一个已知符号、文件、事实或来源，并读回结果 | 查找扩大为关系梳理或出现歧义 |
| `terra-medium` | 梳理常规调用路径，或盘点闭合范围内的文件和引用 | 需要综合多项来源或约束 |
| `terra-high` | 对比多个来源，或跟踪多条显式关系和边界条件 | 证据发生冲突，或多个模块开始实质性交互 |
| `terra-xhigh` | 综合广泛的跨模块证据、多种可能原因或冲突来源 | 变成最困难的单项综合，或需要作出最终判断 |
| `terra-max` | 最困难的单个闭合只读调查或证据综合 | 工作可以拆成独立研究流，或越过最终决策权限边界 |
| `terra-ultra` | 编排至少两个相互独立的只读研究流 | 工作流并不真正独立，或后续需要写入/高风险结论 |

Sol 负责 Luna 机械实现契约之外的判断和写入权限。

| 角色 | 典型任务 | 何时停止或升级 |
| --- | --- | --- |
| `sol-low` | 单个窄范围、低风险，但仍需要少量临场判断的修改或验证步骤 | 任务变成多步骤，或增加多条显式边界条件 |
| `sol-medium` | 仍需本地判断的常规有界实现、评审或验证 | 多条约束或边界条件需要更深入综合 |
| `sol-high` | 仍需本地判断且带有多条显式约束或边界条件的高要求有界工作 | 多个模块、可能原因或冲突证据开始相互作用 |
| `sol-xhigh` | 存在交互信号的复杂跨文件实现、分析、调试、规划或评审 | 证据仍不稳定、风险升高，或需要完整方案设计 |
| `sol-max` | 高风险最终判断、不稳定的 XHigh 证据，或最困难的跨组件架构与完整方案设计 | 任务包含多个真正独立的工作流 |
| `sol-ultra` | 编排至少两个可以独立推进的异常复杂 Sol 工作流 | 实际只有一个困难的闭合任务；此时应使用 `sol-max` |

### 选择示例

| 剩余任务 | 路由 | 原因 |
| --- | --- | --- |
| 在已知文件中重命名指定字段，并通过精确 fixture 验证 | `luna-medium` | 闭合、字面化实现，并有机械校验基准 |
| 在一个模块中实现多条已指定的校验规则和边界条件 | `luna-high` | 存在多条显式规则，但没有跨模块交互 |
| 按完整方案在指定文件间实现状态转换，并有精确测试 | `luna-xhigh` 或 `luna-max` | 文件交互使用 XHigh；大量分支、状态或规则构成最困难闭合任务时使用 Max |
| 查清某个请求字段在闭合调用链中的转换位置 | `terra-medium` | 范围明确的只读关系梳理 |
| 综合多个模块之间相互冲突的行为证据 | `terra-xhigh` | 多模块和冲突证据相互作用，但不把最终决策交给 Terra |
| 实现一个方案仍需局部权衡的闭合功能 | `sol-medium` 到 `sol-xhigh` | 缺少机械闭合条件，不能使用 Luna；effort 按剩余交互复杂度选择 |
| 根据已收集证据决定高风险迁移或安全边界 | `sol-max` | 高风险最终判断不能由 Luna 或 Terra 负责 |
| 并行调查两个相互独立的子系统 | 多个 `terra-medium` | 父 Agent 的普通并行不要求 Ultra |
| 让一个子 Agent 编排异常复杂的研究计划 | `terra-ultra` | 该子 Agent 自身需要协调多个独立证据流 |

同一个未解决任务重试时，只能在家族内单向升级：Luna 使用
`medium -> high -> xhigh -> max`；Terra 和 Sol 使用
`low -> medium -> high -> xhigh -> max`。如果 Luna 失去方案闭合性，或 Terra
需要写入，应切换到 Sol，而不是用更高 effort 弥补模型家族错误。

## 路由凭证与冲突优先级

每个子 Agent 的 prompt 都必须包含紧凑路由凭证：剩余工作、委派收益、阶段、
工作模式、闭合状态、风险、复杂度信号、独立工作流数量、选中的模型与 effort、
拒绝相邻层级的原因以及失败后的 fallback。这样每次降档、继承和升级都有可检查
依据。

冲突处理采用 fail-closed 顺序：授权和单写入者边界优先；先拆分任务，再判断
并行、串行或直接执行；随后选择模型家族和 effort。降档要求所有条件成立，任一
高风险信号都能阻止便宜路由，未知状态不能作为降档证据。`default` 最后考虑，
并且必须证明同层级能力确实必要。

## 按阶段与剩余任务重新评估

路由依据是子 Agent 当前剩余的工作，而不是原任务最困难阶段使用过的模型。
父 Agent 会在设计转实现、实现转验证、探索转决策以及每次任务拆分或交接时
重新评估路由。

设计完成并不自动意味着可以降低模型层级。只有当实现所需的关键决策、接口、
文件边界、验收证据、禁止选择和独立机械校验方式都已经闭合时，才可以降低
层级；这些闭合条件可以由父 Agent 在任务过程中建立。满足后，非琐碎的字面或
重复性实现默认委派给 Luna Medium，多条显式规则实现使用 Luna High，多文件
相互作用实现使用 Luna XHigh，最困难的闭合重逻辑实现使用 Luna Max。仍需临场
判断的实现使用 `sol-low`、`sol-medium` 或 `sol-high`；设计重新出现缺口或跨模块
原因尚未解决时，使用 Sol XHigh 或 Max。

新的、更窄且边界完整的任务可以选择更低层级，但同一个尚未解决任务的重试
仍然只能单向升级。如果子 Agent 执行时发现范围扩大或设计缺口重新出现，必须
把证据交回父 Agent 重新路由，不能静默扩大自己的职责。验证阶段也按完成声明
所需的风险和判断强度独立选型，最终完成声明仍由父 Agent 负责。

## 运行时兼容回退

本地 Codex 模型缓存只作为提示性证据。effort 条目缺失，或缓存损坏、过期、
不完整、尚未生成时，不会让安装器、验证器、委派或父任务失败；可观察到的异常
会被报告为 warning。路由器仍先尝试配置的角色，并以运行时对子 Agent 启动的
实际结果为准。

如果 `ultra` 子 Agent 被运行时拒绝，路由器可以在同一家族内尝试一次 `max`，
由父 Agent 接管编排。其他具名角色失败时，也只允许一次满足风险和验收条件的
同家族安全回退，不能静默跨模型家族，也不能用 `default` 掩盖不兼容。如果回退
仍失败或不存在安全候选，父 Agent 直接执行该任务，避免路由元数据阻塞用户任务。

## 启动信息可见

每次子 Agent 启动时，Codex App 或 CLI 事件流都会显示一条消息：

```text
Subagent started | role: sol-xhigh | model: gpt-5.6-sol | reasoning: xhigh
```

模型值直接取自运行时的 `SubagentStart` 事件，因此反映该子 Agent 实际选择
的模型。具名路由角色的推理强度取自已安装的 TOML 配置。由于该运行时事件
目前不提供推理强度字段，`default` 会如实显示 `inherited from parent`；没有
配置强度的内置或未知角色会显示
`runtime-selected (not exposed by SubagentStart)`，不会猜测具体值。

路由器还会给每个子 Agent 的任务名增加前缀，让模型层级和推理强度直接显示
在 Codex App 的 **Subagents** 列表中。例如：

```text
gpt56_luna_max_analyze_rules
```

当前 App 会把这个标识符显示为列表行标题。开头字段表达的可读文案是
`GPT56 · luna · max`；由于 Codex 任务名只允许小写字母、数字和下划线，实际
参数必须使用下划线编码。这是文本前缀，并非 App 原生 badge。标签根据子
Agent 最终生效的模型和强度生成，不根据角色名生成。完整映射如下：

| 子 Agent 最终家族 | Effort | 任务名前缀格式 |
| --- | --- | --- |
| `gpt-5.6-luna` | 具名角色使用 `medium` 到 `max`；`low` 只用于继承观测 | `gpt56_luna_<effort>` |
| `gpt-5.6-terra` | `low` 到 `max`、`ultra` | `gpt56_terra_<effort>` |
| `gpt-5.6-sol` | `low` 到 `max`、`ultra` | `gpt56_sol_<effort>` |
| 启动前无法取得模型或强度 | — | `runtime_selected` |

`default` 只有在启动前能够明确取得父会话最终模型和强度时才生成具体标签，
否则使用 `runtime_selected`。
生命周期消息仍负责校验 Codex 实际使用的运行时模型。

## 安装

需要 Python 3.11 或更高版本，以及当前版本的 Codex。

从 GitHub 一键安装：

```bash
curl -fsSL https://raw.githubusercontent.com/LAwLi3tCoding/codex-subagent-router/main/install.sh | sh
```

也可以克隆仓库后执行：

```bash
./install.sh
```

出于安全考虑，Codex 会要求用户对新增或发生变化的命令 Hook 做一次信任
确认。安装后请启动 Codex CLI，执行 `/hooks`，信任路由器的
`SubagentStart` Hook，然后重启 App 或 CLI 会话。安装器不会绕过这项官方
安全机制。

### 安装后会发生什么

- 将 Luna、Terra 和 Sol 角色定义安装到用户的 Codex 配置目录。
- 升级时先把已经退役的 pre-canonical 角色文件放入时间戳备份，再从配置目录
  删除。
- 在全局 Codex `AGENTS.md` 中写入一个受管理的路由规则区块，使每个新会话
  都能使用这套路由策略。
- 安装并启用用户级 `SubagentStart` 生命周期 Hook，在每个子 Agent 启动时
  显示角色、运行时实际模型，以及已配置或继承的推理强度。
- 启用多 Agent 能力，并由 Codex 运行时和父 Agent 动态决定并发数量与分批
  方式。
- 清理与会话继承和动态并发冲突的旧版子 Agent 模型覆盖及固定并发覆盖，使
  `default` 使用当前会话选择的模型和推理强度。
- 当旧版全局委派语句设置了固定子 Agent 数量时，将其规范化为动态决策。

### 安全与可恢复性

- 修改已有受管理目标前，创建带时间戳的本地备份。
- 只修改路由器管理的规则区块、已知 Agent 配置项、具名角色文件，以及精确
  匹配的受支持旧版固定数量委派语句；保留所有无关配置和说明。
- 安装过程是幂等的，更新仓库后可以安全地再次运行。
- 只安装 Codex 生命周期 Hook；不会修改 shell 启动文件，也不会运行常驻
  后台进程。

## 验证

```bash
python3 -m unittest discover -s tests -v
python3 scripts/verify.py
```

验证覆盖角色/模型/effort 矩阵、`default` 继承、路由凭证与合成场景、全局受
管理规则、生命周期 Hook 注册、配置解析、安装幂等性和仓库隐私检查。存在
Codex 模型缓存时，不受支持的家族/effort 组合或不可读缓存会作为提示性 warning
输出，不会让验证失败；已安装角色、策略、Hook、配置或隐私检查存在缺陷时仍会
fail-closed。

## 文件结构

- `agents/`：Codex 自定义 Agent 定义。
- `hooks/`：显示子 Agent 启动信息的生命周期 Hook。
- `policy/subagent-routing.md`：全局加载的路由策略。
- `policy/routing-scenarios.json`：合成路由凭证契约场景。
- `scripts/install.py`：幂等的用户级安装器。
- `scripts/verify.py`：安装状态和可公开性检查。
- `tests/`：安装器与路由策略回归测试。

## 发布

仓库不应包含个人路径、账号标识、私有配置、凭证或组织内部信息。发布或推送
到代码托管平台前，请检查完整 Git 历史并运行验证命令。
